...
public class CustomHeaderUtils {
  ...
  public CustomHeaderUtils(String requestMajorVer, Map<String, String> reqHeaderMap, String apiFilePath, String requestedApiName) {
    // all api versions
    if (StringUtils.isNotEmpty(requestedApiName)) {
    ...
    this.minorVersion = (MapUtils.isEmpty(reqHeaderMap)) ? null : reqHeaderMap.get(API_MINIOR_VERSION);
  ...
  private Map<String, String> calCustomHeader() {
    if (CollectionUtils.isEmpty(this.verLst)) {
    ...
    if (StringUtils.isEmpty(this.minorVersion)) {
