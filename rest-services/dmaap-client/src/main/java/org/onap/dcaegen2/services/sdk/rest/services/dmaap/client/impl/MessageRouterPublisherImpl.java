...
import static org.onap.dcaegen2.services.sdk.rest.services.dmaap.client.model.config.TimoutConfig.TIMEOUT_ERROR_PUBLISHER_RESPONSE;
...
public class MessageRouterPublisherImpl implements MessageRouterPublisher {
    ...
    private Publisher<? extends MessageRouterPublishResponse> pushBatchToMr(
        ...
        return httpClient.call(buildHttpRequest(request, createBody(batch, request.contentType())))
                ...
                .doOnError(ReadTimeoutException.class, e -> LOGGER.error("Timout exception occurred when sending items to DMaaP MR", e))
                .onErrorResume(ReadTimeoutException.class, e -> Mono.just(TIMEOUT_ERROR_PUBLISHER_RESPONSE));
    }
    ...
    private @NotNull HttpRequest buildHttpRequest(MessageRouterPublishRequest request, RequestBody body) {
        ...
        return Option.of(request.timoutConfig())
                .map(timoutConfig -> requestBuilder.timeout(timoutConfig.getTimeout()).build())
