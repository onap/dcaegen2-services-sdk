...
public class SimpleHttpClient {
    ...
    public Mono<String> call(HttpRequest request, Charset charset) {
        return call(request, BodyTransformer.asString(charset));
    }

    public <T> Mono<T> call(HttpRequest request, BodyTransformer<T> bodyTransformer) {
        return prepareRequest(request)
                .responseSingle((resp, content) -> HttpStatusClass.SUCCESS.contains(resp.status().code())
                        ? content.as(bodyTransformer)
