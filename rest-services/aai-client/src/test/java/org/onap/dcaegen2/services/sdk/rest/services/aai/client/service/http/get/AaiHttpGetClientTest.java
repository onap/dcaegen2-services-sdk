...
class AaiHttpGetClientTest extends AbstractHttpClientTest {
    ...
    void getAaiResponse_shouldCallGetMethod_withGivenAaiHeaders() {
        ...
        when(httpClient
                .get(anyString(), any(RequestDiagnosticContext.class), eq(headers), eq(String.class)))
                .thenReturn(Mono.just(response));

        StepVerifier
                .create(cut.getAaiResponse(() -> modelId))
                .expectNext(response)
                .verifyComplete();
    }
    ...
    void getAaiResponse_shouldCallGetMethod_withProperUri() {
        ...
        String expectedUri = constructAaiUri(configuration, modelId);

        AaiHttpGetClient cut = new AaiHttpGetClient(configuration, httpClient);

        when(httpClient
                .get(eq(expectedUri), any(RequestDiagnosticContext.class), any(), eq(String.class)))
                .thenReturn(Mono.just(response));
