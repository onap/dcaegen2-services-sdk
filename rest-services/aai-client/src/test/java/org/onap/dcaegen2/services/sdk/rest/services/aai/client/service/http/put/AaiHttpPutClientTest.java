...
class AaiHttpPutClientTest extends AbstractHttpClientTest {
    ...
    void getAaiResponse_shouldCallPutMethod_withGivenAaiHeaders() {
        ...
        when(httpClient
                .put(anyString(), any(RequestDiagnosticContext.class), eq(headers), eq(bodyBuilder), eq(model)))
    ...
    void getAaiResponse_shouldCallPutMethod_withProperUri() {
        ...
        when(httpClient
                .put(eq(uri), any(RequestDiagnosticContext.class), any(), eq(bodyBuilder), eq(model)))
                .thenReturn(Mono.just(response));
