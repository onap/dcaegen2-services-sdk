...
class AaiRequestsBuilder {
    ...
    public <T extends AaiDeletable & AaiRelationable, U extends AaiModel> Mono<HttpResponse>
        buildDeleteRelation(String fromUrl, String toUrl, String aaiTypeName, AaiRelation<T, U> relation) {

        final T from = relation.from();
        final RelationType relType = relation.relationType();

        return findRelatedTo(from.getRelationshipList(), aaiTypeName, relType)
                .map(Relationship::getRelatedLink)
                .flatMap(Option::of)
                .filter(link -> link.endsWith(toUrl))
