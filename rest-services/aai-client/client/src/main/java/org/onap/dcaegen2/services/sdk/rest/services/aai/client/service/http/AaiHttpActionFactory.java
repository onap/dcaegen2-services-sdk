...
public class AaiHttpActionFactory extends AaiHttpAbstractActionFactory {
     ...
     * @param createInsecureClient used for creation of RxHttpClient without SSL.
     * @param createSecuredClient used for creation of RxHttpClient with SSL.
    ...
    protected <T extends AaiRelationable, U extends AaiModel>
        ...
        return new AaiGetRelationAction<T, U>() {
            ...
            public <S extends T> Mono<AaiRelation<S, U>> callT(S model) {
                final Option<Relationship> maybeRel = findRelatedTo(model.getRelationshipList(), getAaiType(to));

                if (maybeRel.isEmpty()) {
                    return Mono.empty();
                }

                final Relationship rel = maybeRel.get();
                final String url = buildUri(rel.getRelatedLink());
                final JsonObject req = toJsonObject(getConverter(), rel);

                return requestsBuilder
                        .buildGetAction(url)
                        .flatMap(AaiHttpActionFactory.this::responseAsJson)
                        .map(obj -> mergeObjects(req, obj))
                        .map(obj -> getConverter().fromJson(obj, to))
                        .map(obj -> AaiRelation.create(model, obj, toRelationType(rel)));
            }
        };
    }
