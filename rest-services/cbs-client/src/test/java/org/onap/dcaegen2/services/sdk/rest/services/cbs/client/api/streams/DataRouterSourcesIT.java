...
class DataRouterSourcesIT {
    ...
    DataRouterSourcesIT() throws IOException {
    }
    ...
    void allStreamsShouldBeOfProperType() {
        assertThat(sources.map(RawDataStream::type).distinct()).containsExactly("data_router", "message_router");
        assertThat(sinks.map(RawDataStream::type).distinct()).containsExactly("data_router");
    }
