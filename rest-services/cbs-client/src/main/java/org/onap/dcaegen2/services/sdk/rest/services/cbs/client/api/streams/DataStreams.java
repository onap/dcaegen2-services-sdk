...
public final class DataStreams {
    ...
    private static Stream<RawDataStream<JsonObject>> createCollectionOfStreams(JsonObject rootJson, DataStreamDirection direction) {
        ...
        return streamsJson == null
                ? Stream.empty()
                : mapJsonToStreams(streamsJson, direction);
    }

    private static Stream<RawDataStream<JsonObject>> mapJsonToStreams(JsonElement streamsJson, DataStreamDirection direction) {
        return Stream.ofAll(streamsJson.getAsJsonObject().entrySet())
                .map(namedSinkJson ->
                        ImmutableRawDataStream.<JsonObject>builder()
                                .direction(direction)
                                .name(namedSinkJson.getKey())
                                .descriptor(namedSinkJson.getValue().getAsJsonObject())
                                .build());
    }
}
