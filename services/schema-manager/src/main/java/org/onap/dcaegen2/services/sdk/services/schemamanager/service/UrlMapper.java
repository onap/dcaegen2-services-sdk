...
public class UrlMapper {
    ...
    public static UrlMapper getInstance(String mappingFilePath, String schemasPath) {
        ...
        try {
            ...
            LOGGER.error("Unable to read mapping file. Mapping file path: {}", mappingFilePath);
        } catch (NullPointerException ex) {
            mappings = Collections.emptyMap();
            LOGGER.warn("Schema mapping file has incorrect format", ex);
        }
    ...
    private static boolean doesLocalFileExist(String schemaRelativePath) {
        if (!FileReader.doesFileExists(schemaRelativePath)) {
            LOGGER.warn("Local resource missing. Schema file with path {} has not been found.", schemaRelativePath);
            return false;
        }
        return true;
    }
