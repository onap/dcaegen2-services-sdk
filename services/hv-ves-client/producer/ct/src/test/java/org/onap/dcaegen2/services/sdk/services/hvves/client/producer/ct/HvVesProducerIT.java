...
class HvVesProducerIT {
    ...
    void singleMessageTest() throws Exception {
        ...
        // check wire protocol
        WireProtocolUnpacker unpacked = WireProtocolUnpacker.unpack(receivedData);
        assertThat(unpacked.type).isEqualTo((short)0x0001);

        // check common header
        assertThat(unpacked.event.getCommonEventHeader().getDomain()).isEqualTo("RTPM");

        // check payload
        MeasDataCollectionOuterClass.MeasDataCollection content =
                MeasDataCollectionOuterClass.MeasDataCollection.parseFrom(unpacked.event.getEventFields());
